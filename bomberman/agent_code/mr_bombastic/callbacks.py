...
def state_to_features(game_state: dict) -> np.array:
    ...
    if len(game_state['bombs'])>0 and np.min(np.linalg.norm(bomb_positions(game_state)-np.array(game_state['self'][3]),axis=1))<=4:
        ...
        if other_bombs(game_state)[0]==1: bomb_right=0
        if other_bombs(game_state)[1]==1: bomb_left=0
        if other_bombs(game_state)[2]==1: bomb_up=0
        if other_bombs(game_state)[3]==1: bomb_down=0
...
def free_space(game_state: dict) -> np.array:
    ...
    new_agent_position=np.array(game_state['self'][3])+np.array([3,3])
    ...
    new_game_field=np.insert(new_game_field,0,-1,axis=1)
    new_game_field=np.insert(new_game_field,18,-1,axis=1)
    new_game_field=np.insert(new_game_field,0,-1,axis=0)
    new_game_field=np.insert(new_game_field,18,-1,axis=0)

    new_game_field=np.insert(new_game_field,0,-1,axis=1)
    new_game_field=np.insert(new_game_field,19,-1,axis=1)
    new_game_field=np.insert(new_game_field,0,-1,axis=0)
    new_game_field=np.insert(new_game_field,19,-1,axis=0)
    ...
    right_5=1-(1-np.abs(new_game_field[new_agent_position[0]+4,new_agent_position[1]])) * (1-np.abs(new_game_field[new_agent_position[0]+3,new_agent_position[1]])) * (1-np.abs(new_game_field[new_agent_position[0]+2,new_agent_position[1]]))
    # Left free space
    ...
    left_5=1-(1-np.abs(new_game_field[new_agent_position[0]-4,new_agent_position[1]])) * (1-np.abs(new_game_field[new_agent_position[0]-3,new_agent_position[1]])) * (1-np.abs(new_game_field[new_agent_position[0]-2,new_agent_position[1]]))
    # Up free space
    ...
    up_5=1-(1-np.abs(new_game_field[new_agent_position[0],new_agent_position[1]-4])) * (1-np.abs(new_game_field[new_agent_position[0],new_agent_position[1]-3])) * (1-np.abs(new_game_field[new_agent_position[0],new_agent_position[1]-2])) 
    # Down free space
    ...
    down_5=1-(1-np.abs(new_game_field[new_agent_position[0],new_agent_position[1]+4])) * (1-np.abs(new_game_field[new_agent_position[0],new_agent_position[1]+3])) * (1-np.abs(new_game_field[new_agent_position[0],new_agent_position[1]+2]))
    # Construct vector
    free_way=np.array([right_1*right_2*right_3*right_4*right_5,left_1*left_2*left_3*left_4*left_5,up_1*up_2*up_3*up_4*up_5,down_1*down_2*down_3*down_4*down_5])
    return free_way
...
def other_bombs(game_state: dict):
    """
    This function takes the current gamestate and returns four number which say in which direction
    the agent would move in another bombing area from a different bomb
    0: for a movement in no other bomb area
    1: for a movement in another bomb area

    :param game_state:  A dictionary describing the current game board.
    :return: four values
    """
    # Calculate relative agent bomb positions
    agent_bomb_position=bomb_positions(game_state)-np.array(game_state['self'][3])
    distance=np.linalg.norm(agent_bomb_position,axis=1)
    smallest_distance_index=np.argmin(distance)
    # Remove the bomb with the smallest distance (because this is the targetbomb anyway)
    agent_bomb_position=np.delete(agent_bomb_position,smallest_distance_index,0)
    right=0
    left=0
    up=0
    down=0
    for i in range(len(agent_bomb_position)):
        if np.linalg.norm(agent_bomb_position[i])<=4:
            right=right + int(agent_bomb_position[i,0]-1==0)
            left=left + int(agent_bomb_position[i,0]+1==0)
            up=up + int(agent_bomb_position[i,1]+1==0)
            down=down + int(agent_bomb_position[i,1]-1==0)
