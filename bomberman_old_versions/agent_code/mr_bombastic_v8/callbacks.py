...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    crate_infront=int(right_block==1 or left_block==1 or up_block==1 or down_block==1)# or len(target_positions(game_state))==0)#the last or is probably not the best 
...
def sourrounding_blocks(game_state: dict):
    agent_position=np.array(game_state['self'][3])
    new_game_field=game_state['field']
