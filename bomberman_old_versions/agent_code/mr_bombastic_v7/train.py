...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state != None:
        if len(old_game_state['bombs'])>0:
            ...
            bomb_position=np.array([np.array(old_game_state['bombs'][0][0])])[samllest_distance_index]
    ...
    if old_game_state != None:
        if len(new_game_state['bombs'])>0:
            distance=np.min(np.linalg.norm(np.array([np.array(new_game_state['bombs'][0][0])-np.array(new_game_state['self'][3])]),axis=1))
            if not old_game_state['self'][2] and distance%1!=0 and e.WAITED in events:
                events.append(NOT_IN_BOMB_AREA_EVENT)
        if len(old_game_state['bombs'])>0:
            distance=np.min(np.linalg.norm(np.array([np.array(old_game_state['bombs'][0][0])-np.array(new_game_state['self'][3])]),axis=1))
            if not old_game_state['self'][2] and distance%1!=0 and e.WAITED in events:
                events.append(NOT_IN_BOMB_AREA_EVENT)
