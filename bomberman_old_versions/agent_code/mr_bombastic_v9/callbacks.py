...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    #Feature 3: Out of bomb area  has to check for all bombs in this range!!!!!!!!!!!!!
    ...
    if len(target_positions(game_state))>0:
        ...
        ####Test
        if len(np.array(game_state['coins']))>0 and np.min(np.linalg.norm(np.array(game_state['coins'])-agent_position,axis=1))<=4.2:
            coin_distance_index=np.argmin(np.linalg.norm(np.array(game_state['coins'])-agent_position,axis=1))
            target_direction_x=(np.array(game_state['coins'])-agent_position)[coin_distance_index,0]
            target_direction_y=(np.array(game_state['coins'])-agent_position)[coin_distance_index,1]
        ####Test
